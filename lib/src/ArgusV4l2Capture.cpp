...
ARGUS_STATE ArgusV4l2Capture::openCamera(const ArgusCameraConfig &config,bool recreate_daemon)
{
    ...
    if (pixel_mode == PixelMode::COLOR_RGB)
        nChannel = 3;

    if (pixel_mode == PixelMode::RAW10 || pixel_mode == PixelMode::RAW12)
    {
        nChannel = 1;
        pixel_depth = 2;
    }
    ...
    while (poll(fds, 1, 5000) > 0 && !exit_)
    {
        if (fds[0].revents & POLLIN) {
            ...
            if (pixel_mode == PixelMode::COLOR_RGB || PixelMode::COLOR_RGBA)
            {
                ...
                c_res = convert( mCvtfd,0,newImage);
            }
            else
                c_res = convert( ctx.g_buff[v4l2_buf.index].dmabuff_fd,0,newImage);
            if (c_res==0)
            {
